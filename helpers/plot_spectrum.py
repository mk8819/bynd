"""Utilities for plotting BYND absorption spectra against RT-TDDFT references."""
...
def read_data(file_name: str) -> tuple[np.ndarray, np.ndarray]:
    """Read a two-column spectrum file (energy, intensity).
    ...
    -------
    omega_array:
        Energy/frequency axis, shape ``(n_points,)``.
    abs_str_array:
        Absorption strength or intensity, shape ``(n_points,)``.
    """
    omega = []
    abs_str = []
...
def plot_simple_spectrum() -> None:
    """Plot BYND and exact RT-TDDFT absorption spectra and save to PDF.

    Reads ``line-search-tddft.abs_strength.dat`` (BYND result) and
    ``rt-tddft.abs_strength.dat`` (long-time RT-TDDFT reference) from the
    working directory, overlays them on a single axes, and writes the figure
    to ``spectrum_bynd_vs_exact.pdf``.
    """
    # settings
